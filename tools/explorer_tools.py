...
def _m7_build_automator_request(
    self,
    *,
    explorer,
    payload: RunExplorerInput,
) -> AutomatorRunRequest:
    instrument_names, select_all = self._parse_instruments(payload.instruments)
...
def _m7_metastock_boundary_tool(
    self,
    payload: RunExplorerInput,
    *,
    tool_name: str,
    ...
    try:
        ...
        if require_validation and not explorer.validation.passed:
            return self._blocked_result(
                tool_name=tool_name,
                code="EXPLORER_VALIDATION_FAILED",
                message="Explorer validation failed; MetaStock action is blocked.",
                title="Explorer Cannot Be Used",
                markdown=(
                    "The Explorer failed validation. Repair it before "
                    "creating or running it in MetaStock."
                ),
                details={
                    "explorer_id": explorer.explorer_id,
                    "validation_errors": explorer.validation.errors,
        ...
        if not explorer.name.strip():
            return self._blocked_result(
                ...
                message="Explorer is missing a name.",
                title="Explorer Cannot Be Used",
                markdown="The Explorer cannot be selected without a name.",
                details={"explorer_id": explorer.explorer_id},
            )

        if require_validation and not explorer.filter_code.strip():
            return self._blocked_result(
                tool_name=tool_name,
                code="EXPLORER_FILTER_MISSING",
                message="Explorer is missing its filter formula.",
                title="Explorer Cannot Be Used",
                markdown=(
                    "The Explorer cannot be created or run because its "
        ...
        if not self.automator_client.configured:
            return self._blocked_result(
                tool_name=tool_name,
                code="AUTOMATOR_NOT_CONFIGURED",
                message="MetaStock automation is not configured.",
                title="MetaStock Automation Not Connected",
                markdown=(
                    "The requested MetaStock action was not performed because "
                    "the AutomatorClient is not connected."
                ),
                details={"explorer_id": explorer.explorer_id},
            )

        request = self._m7_build_automator_request(
            explorer=explorer,
            payload=payload,
        )
        client_method = getattr(self.automator_client, client_method_name)
        automator_result = client_method(request)
        ...
        output = RunExplorerOutput(
            ...
            started_at=automator_result.started_at,
            finished_at=automator_result.finished_at,
            result_available=automator_result.result_available,
            diagnostics=automator_result.diagnostics,
        )
        ...
        return self._exception_result(tool_name=tool_name, exc=exc)


def _m7_create_explorer_in_metastock(
    self,
    payload: RunExplorerInput,
) -> ToolResult:
    return self._m7_metastock_boundary_tool(
        payload,
        tool_name="create_explorer_in_metastock",
        client_method_name="create_explorer",
        success_title="Explorer Created in MetaStock",
        failed_title="MetaStock Explorer Creation Failed",
        require_validation=True,
    )


def _m7_select_explorer_in_metastock(
    self,
    payload: RunExplorerInput,
) -> ToolResult:
    return self._m7_metastock_boundary_tool(
        payload,
        tool_name="select_explorer_in_metastock",
        client_method_name="select_explorer",
        success_title="Explorer Selected in MetaStock",
        failed_title="MetaStock Explorer Selection Failed",
        require_validation=False,
    )


def _m7_run_selected_explorer_in_metastock(
    self,
    payload: RunExplorerInput,
) -> ToolResult:
    return self._m7_metastock_boundary_tool(
        payload,
        tool_name="run_selected_explorer_in_metastock",
        client_method_name="run_selected_explorer",
        success_title="Selected Explorer Run Completed",
        failed_title="MetaStock Selected-Explorer Run Failed",
        require_validation=False,
    )


def _m7_run_explorer_in_metastock_disabled(
    self,
    payload: RunExplorerInput,
) -> ToolResult:
    return ToolResult(
        ...
        status=ToolStatus.BLOCKED,
        message=(
            "Composite run_explorer_in_metastock is disabled. The chat "
            "controller must call select_explorer_in_metastock and "
            "run_selected_explorer_in_metastock as separate tool calls."
        ),
        error=ToolError(
            code="COMPOSITE_TOOL_DISABLED",
            message=(
                "Create, select, and run must remain separate service boundaries."
            ),
        ),
        display=ToolDisplay(
            title="Composite Tool Disabled",
            markdown="Use the separate MetaStock tools: create, select, and run selected.",
            severity="warning",
        ),
    )


ExplorerToolService._m7_build_automator_request = _m7_build_automator_request
ExplorerToolService._m7_metastock_boundary_tool = _m7_metastock_boundary_tool
ExplorerToolService.create_explorer_in_metastock = _m7_create_explorer_in_metastock
ExplorerToolService.select_explorer_in_metastock = _m7_select_explorer_in_metastock
ExplorerToolService.run_selected_explorer_in_metastock = _m7_run_selected_explorer_in_metastock
ExplorerToolService.run_explorer_in_metastock = _m7_run_explorer_in_metastock_disabled
