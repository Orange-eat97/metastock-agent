...
class MainWindow(QMainWindow):
    ...
    def _save_explorer(
        ...
        if self._turn_thread is not None or not self._active_conversation_id:
            return
        if explorer_id.startswith("transient:"):
            backend_text = (
                encode_explorer_upload_envelope(
                    draft_id=explorer_id,
                    name=patch.name,
                    description=patch.description,
                    columns=[
                        {
                            "col_letter": item.label,
                            "col_code": item.formula,
                        }
                        for item in patch.columns
                    ],
                    filter_code=(
                        patch.filter_formula
                    ),
                    assumptions=list(
                        patch.assumptions
                    ),
    ...
    def _start_turn(self, text: str) -> None:
        parsed = parse_explorer_template(text)
        if parsed.matched:
            patch = parsed.patch
            if patch is None:
                raise RuntimeError(
                    "Explorer template parser returned "
                    "no draft."
                )
            backend_text = (
                encode_explorer_upload_envelope(
                    draft_id="transient:template",
                    name=patch.name,
                    description=patch.description,
                    columns=[
                        {
