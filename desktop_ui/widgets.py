...
class ExplorerInlineCard(QFrame):
    def __init__(
        ...
        header = QHBoxLayout()
        title = QLabel(explorer.name or "Explorer")
        ...
        for column in explorer.columns:
            root.addWidget(FormulaBlock(f"Column {column.label}", column.formula))
        root.addWidget(FormulaBlock("Filter", explorer.filter_formula))
    ...
    def __init__(
        ...
        if message.role == "assistant":
            if message.explorer is not None:
                column_layout.addWidget(ExplorerInlineCard(message.explorer))
...
class ChatArea(QFrame):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        input_layout.addWidget(self._send_button, 0, Qt.AlignmentFlag.AlignBottom)
    ...
    def add_message(self, message: ChatMessageViewModel) -> None:
