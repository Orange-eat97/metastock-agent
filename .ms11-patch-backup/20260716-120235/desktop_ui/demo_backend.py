...
class DemoConversationBackend(ConversationBackendPort):
    ...
    def save_explorer_edits(
        ...
        for snapshot in self._snapshots.values():
            for message in snapshot.messages:
                ...
                if explorer is None or explorer.explorer_id != explorer_id:
                    continue
                if explorer.manual_edit_version != expected_version:
                    raise RuntimeError(
                        "This Explorer changed after the card was opened. Reload it before saving."
                    )
                explorer.name = patch.name
                explorer.description = patch.description
                explorer.columns = list(patch.columns)
                explorer.filter_formula = patch.filter_formula
                explorer.assumptions = list(patch.assumptions)
                explorer.manual_edit_version += 1
                explorer.updated_at = now_text()
                snapshot.active_explorer = copy.deepcopy(explorer)
                return copy.deepcopy(explorer)
        raise ValueError("Explorer not found.")
    ...
    def export_conversation_markdown(
        ...
        snapshot = self._snapshots[conversation_id]
        user_turns = sum(1 for item in snapshot.messages if item.role == "user")
        assistant_turns = sum(
            1 for item in snapshot.messages if item.role == "assistant"
        )
        lines = [
            f"# {snapshot.title}",
            "",
            ...
            f"- Assistant messages: {assistant_turns}",
            f"- Explorer cards: {sum(1 for item in snapshot.messages if item.explorer)}",
            f"- Result cards: {sum(len(item.results) for item in snapshot.messages)}",
            "",
            "## Transcript",
            "",
        ]
