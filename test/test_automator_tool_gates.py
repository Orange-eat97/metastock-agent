...
def build_row(*, validation_passed: bool) -> dict[str, Any]:
    return {
        "id": "explorer-1",
        ...
        "validation_errors": [] if validation_passed else ["Invalid formula"],
        "service_log_id": "log-1",
    }


def build_service(repository: FakeExplorerRepository) -> ExplorerToolService:
    return ExplorerToolService(
        review_workflow=object(),  # run gate does not use generation workflow
...
def test_unknown_explorer_returns_failed() -> None:
    service = build_service(
        FakeExplorerRepository(
            error=ValueError("No explorer_outputs row found for id=missing")
        )
    )

    result = service.run_explorer_in_metastock(
        RunExplorerInput(explorer_id="missing")
    )
...
def test_invalid_explorer_is_blocked_before_automator_gate() -> None:
    service = build_service(
        FakeExplorerRepository(row=build_row(validation_passed=False))
    )
    ...
    assert result.error.code == "EXPLORER_VALIDATION_FAILED"


def test_valid_explorer_is_blocked_when_automator_is_unavailable() -> None:
    service = build_service(
        FakeExplorerRepository(row=build_row(validation_passed=True))
    )

    result = service.run_explorer_in_metastock(
        RunExplorerInput(explorer_id="explorer-1", instruments="all")
    )

    assert result.ok is False
    assert result.status is ToolStatus.BLOCKED
    assert result.error is not None
    assert result.error.code == "AUTOMATOR_NOT_CONFIGURED"
